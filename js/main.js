...
(function () {
  ...
  function initAboutMarkAnimation() {
    ...
    gsap.set(originDot, {
      opacity: 0,
      scale: 0,
      transformOrigin: "99.7px 55.09px",
    });
    ...
    // Speed up the full about mark sequence so it feels snappier.
    tl.timeScale(2.35);

    /* Phase 1 — origin spark */
    tl.to(
      originDot,
      { opacity: 1, scale: 2.4, duration: 0.25, ease: "power2.out" },
      0,
    ).to(
      originDot,
